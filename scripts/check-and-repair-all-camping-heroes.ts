...
const OUT_DIR = path.resolve(process.cwd(), "data/review/camping-hero-health-2026-03-17");
...
async function fetchWithTimeout(url: string, headers: Record<string, string> = {}) {
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        Accept: "image/*,*/*;q=0.8",
        "User-Agent": "camping-portal-all-camping-hero-check/1.0",
        ...headers,
      },
...
async function discoverCandidates(place: PlaceRow, googleKey: string): Promise<HeroCandidateRecord[]> {
  const places = await fetchGooglePlaces(place.name, place.lat, place.lng, googleKey);
  ...
  const candidates = places
    ...
    .filter((entry) => entry.candidateName && entry.overlap >= 0.34)
    .sort((a, b) => {
    ...
    .flatMap((entry) =>
      (Array.isArray(entry.candidatePlace.photos) ? entry.candidatePlace.photos : []).slice(0, 4).map((photo, index) => {
        ...
        const distanceBonus =
          entry.distance == null ? 0 : entry.distance <= 200 ? 20 : entry.distance <= 800 ? 10 : entry.distance <= 2500 ? 4 : 0;
        return {
...
async function main() {
  ...
  if (!googleKey) {
    throw new Error("Missing Google Places key");
  }
