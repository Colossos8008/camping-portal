...
import { buildPlaceHeroProxyPath, isGooglePlacesPhotoUrl, normalizePlaceHeroImageUrlForPublic } from "../src/lib/hero-image.ts";
...
function run() {
  ...
  const wikimedia = "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Test.jpg/1280px-Test.jpg";
  assert.equal(normalizePlaceHeroImageUrlForPublic(70, wikimedia), wikimedia, "Wikimedia URL should pass through");

  const imageUrlHelperSource = fs.readFileSync("src/app/map/_lib/image-url.ts", "utf8");
  assert.equal(
    imageUrlHelperSource.includes("isGooglePhotoReference(cleanPath) || isGooglePlacesPhotoUrl(cleanPath)"),
    true,
