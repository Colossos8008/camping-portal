...
const OUT_DIR = path.resolve(process.cwd(), "data/review/stellplatz-hero-health-2026-03-17");
...
async function fetchWithTimeout(url: string, headers: Record<string, string> = {}) {
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        Accept: "image/*,*/*;q=0.8",
        "User-Agent": "camping-portal-all-stellplatz-hero-check/1.0",
        ...headers,
      },
...
async function discoverCandidates(place: PlaceRow, googleKey: string): Promise<HeroCandidateRecord[]> {
  ...
  return places
    ...
    .filter((entry) => entry.candidateName && entry.overlap >= 0.28)
    .sort((a, b) => {
